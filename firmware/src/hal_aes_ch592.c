 ...
 *
 * hal_aes.h over the CH592's undocumented hardware AES-128 engine.
 *
 * PROVENANCE: ported from OpenDongle (Apache-2.0, same authorship) at
 * `firmware/ch592/src/hal_aes_ch592.c`, branch `em-rf-ccm-rx-decrypt`, where it
 * is bench-validated on this same silicon. The register sequence is unchanged;
 * only comment references are re-pointed at this tree. Keep the two copies in
 * step -- they must produce identical ciphertext or the link fails to decrypt.
 *
 ...
 * into its gptrAESReg global, and reading that global from a running part
 * returns 0x4000C300. (This build links that archive and reaches that init via
 * CH59x_BLEInit() + RF_RoleInit() in main.c, and its map places gptrAESReg in
 * .bss -- so the precondition below is already satisfied here.) The sequence
 * passes FIPS-197 C.1, the all-zero vector, a key-avalanche check and a decrypt
 ...
 * itself on an 875 us poll grid with a 100 us response turnaround, and cannot
 * have a crypto call reach in and cancel a transmit. Driving the registers
 * directly is the same engine without that side effect. (LL_Encrypt also sets
 * bit 7 of the LLE register at 0x4000C250 every call; the engine was verified to
 * work without it, and this firmware leaves that bit clear.) The raw
 * AES_DevAESEnc() entry point in the same archive avoids phy_status_clear but
 * carries the busy-wait trap below, so it is no better.
 *
 ...
 * OpenDongle's tests/test_aes_ch592_mock.py. Nothing but a test should define
 * this.
 */
#ifndef AES_BASE
 ...
 * hung keyboard. A real operation completes in tens of cycles. */
#define AES_SPIN_LIMIT 100000u

/* Per-block interrupt mask (see the note in hal_aes_encrypt_block). Host
 * builds (mocked-register tests) have no mstatus; the mask is a no-op there,
 * which is also correct -- nothing preempts a host harness. */
#if defined(__riscv)
...
void hal_aes_init(void)
{
     ...
     * shipping configuration (CH59x_BLEInit() -> RF_RoleInit() ->
     * BLE_IPCoreInit() has run by the time RF starts), and it answers correctly
     * from a cold reset on this family. */
}
...
void hal_aes_set_key(const uint8_t key[HAL_AES_KEY_BYTES])
{
     ...
     * the work: with the key in flash it measured 1,359 cycles on CH592,
     * against ~120-800 with the key in SRAM. Production keys come from the bond
     * record in RAM, so the SRAM regime is the real one. */
                                       ...
                                       uint8_t out[HAL_AES_BLOCK_BYTES])
{
     ...
     * Root cause (docs/TODO.md section 0, bench 2026-08-15/16): a BLEB
     * preempt mid-operation silently aborts the engine -- CFG bit 0 reads
     * back clear as if complete, but the computation never ran and the DATA
     * read path still holds the PREVIOUS block's output (separate in/out
     * latches; the mocked-register test below sees the input instead, which
     * is why this failure mode stayed invisible to it). The linked
     * BB_IRQLibHandler reads/clears AES_STA, the register this sequence arms.
     *
    ...
    /*
     * Distinguish completion from expiry. Falling straight into the copy below
     * in both cases would return the contents of the data registers with no
     * indication anything was wrong.
     *
     * Measured, not assumed: without this check, this returns THE PLAINTEXT
     * (OpenDongle's tests/test_aes_ch592_mock.py reverts the branch and observes
     * it). The input is written into the data registers just above, so if the
     * engine never runs, reading them back yields exactly what was written. In
     * counter mode the caller XORs that "keystream" with the same plaintext and
