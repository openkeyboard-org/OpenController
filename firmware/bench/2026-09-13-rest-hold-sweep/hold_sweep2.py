...
def phase_charge(label,H,n,tp):
    ...
    emit(f"{label} CHARGE n={n}: excess mC mean={st.mean(ex):.1f} min={min(ex):.1f} max={max(ex):.1f} sd={st.pstdev(ex):.2f} | hold mean={st.mean(hb):.2f}s | recon ms med={st.median(rc):.0f} max={max(rc):.0f} | lock s: {('med=%.0f max=%.0f'%(st.median(lk),max(lk))) if lk else 'none'} certified {len(lk)}/{n}")
