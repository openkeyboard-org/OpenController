...
make factory MRS_TOOLCHAIN="$MRS" OPENBOOT_TOOLCHAIN="$OB" MINICHLINK="$MC" $REST > "$S/flash_build.log" 2>&1 || { echo "build failed:"; tail -5 "$S/flash_build.log"; exit 1; }
grep -E "rf_task|error|Error|factory image:" "$S/flash_build.log" | tail -3   # an up-to-date build prints nothing: never gate on this grep
...
HOLDPID=$!
make flash-factory KBD_PROBE=$KBD_PROBE ALLOW_BONDED_FLASH=1 MRS_TOOLCHAIN="$MRS" OPENBOOT_TOOLCHAIN="$OB" MINICHLINK="$MC" $REST > "$S/flash_flash.log" 2>&1 || { echo "flash failed:"; tail -6 "$S/flash_flash.log"; wait $HOLDPID 2>/dev/null; exit 1; }
grep -iE "read back|WARNING|bond" "$S/flash_flash.log" | tail -3
